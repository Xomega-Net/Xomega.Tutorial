...
namespace AdventureWorks.Services.Common
{
    ...
    public class SalesOrderServiceClient : HttpServiceClient, ISalesOrderService
    {
        ...
        {
            HttpRequestMessage msg = new HttpRequestMessage(HttpMethod.Get, $"sales-order/{ _salesOrderId }/reason/{ _salesReasonId }");
            using (var resp = await Http.SendAsync(msg, HttpCompletionOption.ResponseHeadersRead, token))
            {
                var content = await resp.Content.ReadAsStreamAsync();
                return await JsonSerializer.DeserializeAsync<Output<SalesOrderReason_ReadOutput>>(content, SerializerOptions);
            }
        }
        ...
        {
            HttpRequestMessage msg = new HttpRequestMessage(HttpMethod.Post, $"sales-order/{ _salesOrderId }/reason/{ _salesReasonId }")
            {
                Content = new StringContent(JsonSerializer.Serialize(_data), Encoding.UTF8, "application/json")
            };
            using (var resp = await Http.SendAsync(msg, HttpCompletionOption.ResponseHeadersRead, token))
        ...
        {
            HttpRequestMessage msg = new HttpRequestMessage(HttpMethod.Put, $"sales-order/{ _salesOrderId }/reason/{ _salesReasonId }")
            {
                Content = new StringContent(JsonSerializer.Serialize(_data), Encoding.UTF8, "application/json")
            };
            using (var resp = await Http.SendAsync(msg, HttpCompletionOption.ResponseHeadersRead, token))
        ...
        {
            HttpRequestMessage msg = new HttpRequestMessage(HttpMethod.Delete, $"sales-order/{ _salesOrderId }/reason/{ _salesReasonId }");
            using (var resp = await Http.SendAsync(msg, HttpCompletionOption.ResponseHeadersRead, token))
            {
                var content = await resp.Content.ReadAsStreamAsync();
                return await JsonSerializer.DeserializeAsync<Output>(content, SerializerOptions);
        ...
        {
            HttpRequestMessage msg = new HttpRequestMessage(HttpMethod.Get, $"sales-order/{ _salesOrderId }/reason");
            using (var resp = await Http.SendAsync(msg, HttpCompletionOption.ResponseHeadersRead, token))
            {
                var content = await resp.Content.ReadAsStreamAsync();
                return await JsonSerializer.DeserializeAsync<Output<ICollection<SalesOrderReason_ReadListOutput>>>(content, SerializerOptions);
            }
        }
    }
}