...
namespace AdventureWorks.Services.Common
{
    ...
    ///</summary>
    public class SalesTerritoryServiceClient : RestApiClient, ISalesTerritoryService
    {
        public SalesTerritoryServiceClient(IHttpClientFactory httpClientFactory, RestApiConfig apiConfig,
        ...
        {
            HttpRequestMessage msg = new HttpRequestMessage(HttpMethod.Get, $"sales-territory/enum");
            using (var resp = await Http.SendAsync(msg, HttpCompletionOption.ResponseHeadersRead, token))
            {
                ...
                return JsonSerializer.Deserialize<Output<ICollection<SalesTerritory_ReadEnumOutput>>>(content, SerializerOptions);
            }
        }
    }
}